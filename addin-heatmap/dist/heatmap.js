...
geotab.addin.heatmap = function () {
    // (all your existing variables and functions unchanged)
    var v, h, m, p, y, E, w, i, l, r, c, D, t, I = 5e4;
    ...
    var n = function () {
        ...
        if (null !== t && "" !== i && "" !== l) {
            ...
            v.multiCall(d, function (e) {
                ...
                lastHeatmapPoints = t; // <-- store latest points for table/export
                0 < t.length ?
                    (h.fitBounds(n),
                        m.setLatLngs(t),
                    ...
                    if (0 < t.length) {
                        h.fitBounds(n),
                            ...
                            0 < s || 0 < a && (function () {
                                ...
                                0 < s && 0 < a && (d += " and");
                                a && (d += " the result limit of \n              ".concat(S(I), " was exceeded for \n              ").concat(S(exceededResultsLimitCount), " of the selected vehicles."));
                                b(d += ".");
                            })(), T(!1)
    ...
    var a = function (e) {
        ...
        var t = new Date, n = t.getDate(), o = t.getMonth() + 1, a = t.getFullYear();
        n < 10 && (n = "0" + n), o < 10 && (o = "0" + o),
            E.value = a + "-" + o + "-" + n + "T00:00",
            w.value = a + "-" + o + "-" + n + "T23:59",
        ...
        // --- NEW: Add table and export buttons ---
        ...
        // If buttons exist, wire up listeners
        if (showRawDataBtn) {
        ...
        if (downloadRawDataBtn) {
            downloadRawDataBtn.addEventListener('click', function () {
                if (!lastHeatmapPoints.length) return;
    ...
    function renderRawDataTable(data) {
        if (!data.length) {
            rawDataTableDiv.innerHTML = '<p>No data for selected range.</p>';
            return;
        }
        let html = '<table><thead><tr><th>Latitude</th><th>Longitude</th><th>Timestamp</th></tr></thead><tbody>';
        data.forEach(pt => {
            html += `<tr><td>${pt.lat || pt.latitude || ''}</td><td>${pt.lon || pt.lng || pt.longitude || ''}</td><td>${pt.timestamp || pt.dateTime || ''}</td></tr>`;
        });
        html += '</tbody></table>';
